...
export function Bookshelf({ books }) {
  ...
  const bookshelfRef = React.useRef(null);
  const viewportRef = React.useRef(null);
  const scrollRightRef = React.useRef(null);
  const scrollLeftRef = React.useRef(null);
  ...
  React.useEffect(() => {
    if (router.query.slug && router.query.slug.length > 0 && bookIndex === -1) {
      const idx = books.findIndex((b) =>
        b.slug
          .toLowerCase()
          .includes((router.query.slug)[0].toLowerCase())
      );
      setBookIndex(idx);
    }
  ...
  React.useEffect(() => {
    ...
    // check if current scroll ref exists before adding event listeners
...
Bookshelf.propTypes = {
  books: PropTypes.arrayOf(
    PropTypes.shape({
      ...
      spineColor: PropTypes.string.isRequired,
